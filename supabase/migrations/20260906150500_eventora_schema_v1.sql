...
-- EVENTORA — Supabase/PostgreSQL MVP Schema v1.0
-- Generated from the Eventora Product Bible.
-- Run in a fresh Supabase project or adapt migrations carefully.
-- NOTE: KYC files must use private storage buckets and strict policies.
...
values
('Production & AV','production-av','Event production and audiovisual services',1),
('Photography & Media','photography-media','Photography, video and content services',2),
('Entertainment','entertainment','DJs, bands, MCs and performers',3),
('Creative & Branding','creative-branding','Branding, decoration and creative services',4),
('Rentals','rentals','Equipment and event infrastructure rentals',5),
('Hospitality','hospitality','Catering, ushers and hospitality',6),
('Logistics','logistics','Security, transportation and event logistics',7),
('Venues','venues','Event venues and event spaces',8)
...
cross join lateral (select id from public.categories where slug='production-av') c
on conflict(slug) do nothing;
...
cross join lateral (select id from public.categories where slug='photography-media') c
on conflict(slug) do nothing;
...
cross join lateral (select id from public.categories where slug='entertainment') c
on conflict(slug) do nothing;

-- Review categories
insert into public.review_categories(name,category_type)
values
('Quality','general'),
...
create policy "public can view public equipment"
on public.vendor_equipment for select
...
-- Vendor ownership
create policy "vendor owners can manage vendors"
on public.vendors for all
using (owner_id = auth.uid())
with check (owner_id = auth.uid());

create policy "vendor members can view vendor"
on public.vendors for select
using (
  owner_id = auth.uid()
  or exists (
    select 1 from public.vendor_members vm
    where vm.vendor_id = vendors.id and vm.user_id = auth.uid()
  )
  or (status='active' and verification_status='approved')
);

create policy "vendor owners can manage services"
on public.services for all
using (
  exists(select 1 from public.vendors v where v.id=services.vendor_id and v.owner_id=auth.uid())
)
with check (
  exists(select 1 from public.vendors v where v.id=services.vendor_id and v.owner_id=auth.uid())
);

create policy "vendor owners can manage rentals"
on public.rental_items for all
using (
  exists(select 1 from public.vendors v where v.id=rental_items.vendor_id and v.owner_id=auth.uid())
)
with check (
  exists(select 1 from public.vendors v where v.id=rental_items.vendor_id and v.owner_id=auth.uid())
);
...
-- Events
create policy "clients manage own events"
...
create policy "event owners manage requirements"
on public.event_requirements for all
using (
  exists(select 1 from public.events e where e.id=event_requirements.event_id and e.organizer_id=auth.uid())
)
with check (
  exists(select 1 from public.events e where e.id=event_requirements.event_id and e.organizer_id=auth.uid())
);
...
-- Saved vendors
create policy "users manage saved vendors"
...
create policy "vendors view incoming requests"
on public.booking_requests for select
using (
  exists(select 1 from public.vendors v where v.id=booking_requests.vendor_id and v.owner_id=auth.uid())
);
...
create policy "vendors manage their quotes"
on public.quotes for all
using (
  exists(select 1 from public.vendors v where v.id=quotes.vendor_id and v.owner_id=auth.uid())
)
with check (
  exists(select 1 from public.vendors v where v.id=quotes.vendor_id and v.owner_id=auth.uid())
);
...
on public.bookings for select
using (
  exists(select 1 from public.vendors v where v.id=bookings.vendor_id and v.owner_id=auth.uid())
);
...
create policy "users view recommendations for own events"
on public.ai_recommendations for select
using (
  exists(select 1 from public.events e where e.id=ai_recommendations.event_id and e.organizer_id=auth.uid())
);

-- =========================================================
-- NOTES
-- =========================================================
-- 1. Admin access should be implemented with a secure admin-role
--    mechanism (custom JWT claims or a dedicated admin_roles table).
-- 2. Do not expose vendor_kyc, kyc_documents, payments, payouts,
--    disputes, or audit_logs through public client-side queries.
-- 3. Add Storage policies separately after creating buckets.
-- 4. Financial operations and booking transitions should be performed
--    through server-side functions/Edge Functions, not trusted to the client.
